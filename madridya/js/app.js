...
// ── MARKETPLACE FILTER ────────────────────────
function filterMkt(btn) {
  document.querySelectorAll('.cat-chip').forEach(c => c.classList.remove('sel'));
  btn.classList.add('sel');
  const cat = btn.dataset.cat;

  document.querySelectorAll('.mkt-card, .h-card, .guide-card').forEach(card => {
  ...
  const sections = ['mkt-featured', 'mkt-experiences', 'mkt-guides'];
  sections.forEach(id => {
    const sec = document.getElementById(id);
    if (!sec) return;
    const visible = sec.querySelectorAll('.mkt-card:not([style*="none"]), .h-card:not([style*="none"]), .guide-card:not([style*="none"])');
